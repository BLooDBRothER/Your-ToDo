...
const Folder = ({ id, name, openModal }: FolderPropType) => {
    ...
    return (
        <Dropdown menu={{ items: dropdownMenuItem, onClick: handleDropdownMenuClick }} open={isDropDownOpen} trigger={["contextMenu", "click"]}>
            <div className='bg-primary p-4 flex flex-col items-center gap-4 w-[150px] rounded-lg text-lg hover:bg-primary/80 sm:w-[250px] sm:flex-row' ref={dropCntRef} role='button' onContextMenu={triggerDropdown} title={name}>
                ...
                <div className='sm:flex-1 flex items-center'>
                    ...
                    <div className='hover:bg-light/10 rounded-md z-10' onClick={() => { setDropDownOpen(prev => !prev) }} role='button'>
                        <MoreOutlined />
                    </div>
                </div>
            </div>
